...
import static com.koushikdutta.async.http.spdy.okhttp.internal.Util.checkOffsetAndCount;
import static com.koushikdutta.async.http.spdy.okio.Util.reverseBytesLong;
...
public final class Buffer implements BufferedSource, BufferedSink, Cloneable {
  ...
  @Override public byte readByte() {
    ...
    if (pos == limit) {
      head = segment.pop();
      SegmentPool.getInstance().recycle(segment);
    } else {
      segment.pos = pos;
    }
  ...
  @Override public int read(byte[] sink, int offset, int byteCount) {
    ...
    if (s.pos == s.limit) {
      this.head = s.pop();
      SegmentPool.getInstance().recycle(s);
    }
  ...
  Segment writableSegment(int minimumCapacity) {
    ...
    if (head == null) {
      head = SegmentPool.getInstance().take(); // Acquire a first segment.
      return head.next = head.prev = head;
    }

    Segment tail = head.prev;
    if (tail.limit + minimumCapacity > Segment.SIZE) {
      tail = tail.push(SegmentPool.getInstance().take()); // Append a new empty segment to fill up.
    }
    return tail;
  }
