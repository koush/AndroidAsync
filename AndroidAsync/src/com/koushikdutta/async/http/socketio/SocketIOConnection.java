...
class SocketIOConnection {
	...
	public SocketIOConnection(AsyncHttpClient httpClient,
			SocketIORequest request) {
	...
	public void emitRaw(int type, SocketIOClient client, String message,
		...
		transport.send(String.format(Locale.ENGLISH, "%d:%s:%s:%s", type, ack,
				...
				.then(new TransformFuture<SocketIOTransport, String>() {
					...
					protected void transform(String result) throws Exception {
						...
						if (set.contains("websocket")) {
									...
									.setCallback(
											new FutureCallback<WebSocket>() {
												...
												public void onCompleted(
													...
													transport
															.setComplete(new WebSocketTransport(
																	result,
																	sessionId));
												}
											});
							...
							final String sessionUrl = Uri
									.parse(request.getUri().toString())
									.buildUpon().appendPath("xhr-polling")
									.appendPath(sessionId).build().toString();
							XHRPollingTransport xhrPolling = new XHRPollingTransport(
									httpClient, sessionUrl, sessionId);
	...
	void setupHeartbeat() {
		...
		Runnable heartbeatRunner = new Runnable() {
			@Override
			public void run() {
				if (heartbeat <= 0 || ts != transport || ts == null
						|| !ts.isConnected())
					return;
				transport.send("2:::");
	...
	private void delayReconnect() {
		...
		if (request.config.reconnectDelayMax > 0L) {
			reconnectDelay = Math.min(reconnectDelay,
					request.config.reconnectDelayMax);
		}
	}
	...
	private void reportEvent(String endpoint, final String event,
			final JSONArray arguments, final Acknowledge acknowledge) {
		select(endpoint, new SelectCallback() {
			@Override
			public void onSelect(SocketIOClient client) {
	...
	private Acknowledge acknowledge(final String _messageId,
		...
		return new Acknowledge() {
			...
			public void acknowledge(JSONArray arguments) {
				...
				if (transport == null) {
					final Exception e = new SocketIOException(
